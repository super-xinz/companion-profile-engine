...
const assetDescriptions = {
  ...
  enneagram: "规定9类动机模式、18组相邻特征、6组互动偏好、54种组合、场景策略和维护边界。"
};
const keyLabels = {
  ...
  identity: "基本身份", birth_analysis: "背景规则参考", mbti_dimensions: "认知与决策连续维度",
  ...
  core_types: "9类动机模式", wings: "18组相邻特征", instinct_stacks: "6组互动偏好",
  scene_adaptation: "场景适配层", layer_model: "六层人格模型", weights: "继承权重",
  maintenance: "辅助模型维护规则", boundaries: "模型边界", identity_schema: "辅助分类结构"
};
...
function renderValidation(report) {
  ...
  node.innerHTML = report.valid
    ? `✓ 检查通过：${checks.trait_count ?? 17} 个画像维度、${checks.dialogue_mapping_count ?? 17} 个对话映射、${checks.enneagram_core_type_count ?? 9} 类动机模式、${checks.enneagram_wing_count ?? 18} 组相邻特征、${checks.enneagram_resolved_combination_count ?? 54} 种辅助组合、${checks.conflict_count ?? 0} 个冲突。`
